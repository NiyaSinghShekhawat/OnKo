...
export async function replyToDoctorQuery(input:{queryId:string;message:string}):Promise<Query>{const r=await authenticatedFetch("/api/doctor/queries",{method:"POST",body:JSON.stringify(input)});if(!r.ok)throw new Error("Unable to send reply.");return(await r.json()).data as Query}
