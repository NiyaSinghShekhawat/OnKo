...
export async function createPatientNotification(input: { patientId: string; type: NotificationType; title: string; message: string; channel?: Notification["channel"]; metadata?: Record<string, string> }) {
  const notification: Notification = {
    ...
    patientId: input.patientId, type: input.type, title: input.title, message: input.message,
    createdAt: new Date().toISOString(), channel: input.channel || "in-app", status: "pending", metadata: input.metadata,
  };
  await createDocument("notifications", notification);
  return notification;
}
export async function listPatientNotifications(patientId: string) {
  return (await listDocumentsByField<Notification>("notifications", "patientId", patientId))
    .sort((a,b)=>new Date(b.createdAt).getTime()-new Date(a.createdAt).getTime());
}
export async function markPatientNotificationRead(notificationId:string, patientId:string) {
  const ref=documentRef<Notification>("notifications",notificationId);
  const snap=await ref.get();
  if(!snap.exists) throw new Error("Notification not found.");
  const notification=snap.data() as Notification;
  if(notification.patientId!==patientId) throw new Error("Not authorized.");
  const readAt=new Date().toISOString();
  await ref.update({status:"read",readAt});
